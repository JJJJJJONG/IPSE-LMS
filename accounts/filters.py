...
class StudentFilter(django_filters.FilterSet):
    ...
    name = django_filters.CharFilter(field_name="student__name", method="filter_by_name", label="")
    email = django_filters.CharFilter(field_name="student__email", lookup_expr="icontains", label="")

    class Meta:
        model = Student
        fields = ["id_no", "name", "email"]

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.filters["id_no"].field.widget.attrs.update({"class": "au-input", "placeholder": "학번 (ID No.)"})
        self.filters["name"].field.widget.attrs.update({"class": "au-input", "placeholder": "이름"})
        self.filters["email"].field.widget.attrs.update({"class": "au-input", "placeholder": "이메일"})
